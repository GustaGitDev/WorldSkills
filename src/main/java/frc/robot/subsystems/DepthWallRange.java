    ...
    public void update() {
        ...
        for (int y = y0; y <= y1; y += step) {
            ...
            for (int x = x0; x <= x1; x += step) {
                int idx = rowBase + x * 2;
                if (idx + 1 >= depth.length) continue;
                int d1 = depth[idx] & 0xFF;
                int d2 = depth[idx + 1] & 0xFF;
                int mm = (d2 << 8) | d1;
                if (mm > 0) {
                    if (k < samples.length) samples[k++] = mm;
                }
            }
        }
        if (k == 0) return;
        ...
        if (filteredMeters < 0) filteredMeters = meters;
        else filteredMeters = emaAlpha * meters + (1 - emaAlpha) * filteredMeters;

        lastUpdate = Timer.getFPGATimestamp();
    }

    public double getFilteredDistanceMeters() { return filteredMeters; }
    public double getLastUpdateTime()         { return lastUpdate; }
    public boolean hasValidReading()          { return filteredMeters > 0; }
}
